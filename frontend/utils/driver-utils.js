...
export async function retrieveCompletedTripsForDriver(driverID) {
    const response = await axios.get(`${baseUrl}/trips?driver_id=${driverID}&trip_progress=4`, requestConfig)

    return response.data
}

export async function retrieveAvailableTripsForDriver() {
    const response = await axios.get(`${baseUrl}/trips?trip_progress=2`, requestConfig)
