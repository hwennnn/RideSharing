...
It creates a driver in MySQL database by specific driverID. Infomration such as driver_id, first_name, last_name, mobile_number, email_address, identification_number, car_license_number must be supplied in the request body during registration.
...
```bash
http://localhost:80/server/api/v1/drivers/5
```
...
It create a passenger in MySQL database by specific passengerID. Infomration such as passenger_id, first_name, last_name, mobile_number and email_address must be supplied in the request body during registration.
...
```bash
http://localhost:80/server/api/v1/passengers/5
```
...
```json
201 - Passenger added: 5
202 - Passenger updated: 5
422 - Please supply passenger information in JSON format
422 - The data in body and parameters do not match
```
...
|    trip_progress    |    int    |                       **4 -> The trip has ended (Driver has ended the trip)**                       |
